...
def fast_orth_quant(coeffs, n, hat_coeffs, error, decomp, iter):
    ...
    if n == 8:
        J = ((basis_mats.to(H_sqrt.dtype) @ H_sqrt).unsqueeze(1) * basis_mats.unsqueeze(0)).sum(dim=(-2,-1)) / tr_H_sqrt / 64
        cliques_flat = []
        for c in cliques:
            cliques_flat.extend(c)
        cliques_flat = torch.tensor(cliques_flat)
        J = J[cliques_flat][:, cliques_flat]
        L_J = torch.linalg.cholesky(J)
        coeffs_c = coeffs[cliques_flat]
        hat_c = torch.zeros(64, dtype=coeffs.dtype, device=coeffs.device)
        err_c = torch.zeros(64, dtype=coeffs.dtype, device=coeffs.device)
        for c in range(7, -1, -1): # processes cliques in reverse order
            currc, donec = c * 8, (c + 1) * 8
            if donec < 64: # if not the last clique
                correction = L_J[donec:, currc:donec].T @ err_c[donec:]
            else:
                correction = torch.zeros(8, dtype=coeffs.dtype, device=coeffs.device)
            target = coeffs_c[currc:donec] + correction
            hat_c[currc:donec] = cb.quantize(target.unsqueeze(0).float())[0].squeeze(0).to(coeffs.dtype)
            err_c[currc:donec] = coeffs_c[currc:donec] - hat_c[currc:donec]
        for i, c in enumerate(cliques_flat):
            hat_coeffs[c] = hat_c[i]
            error[c] = err_c[i]
        return
    next_n = n // 2
