    ...
    def __init__(self, params, lr=0.001, momentum=0.9, num_bits=1, num_flips=1):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                basis_param = torch.nn.Parameter(torch.zeros(group["num_bits"], dtype=torch.float32, device=p.device), requires_grad=True)
                ...
                with torch.no_grad():
                    numerator = torch.sum(p.data * raw_bases[0])
                    denominator = torch.sum(raw_bases[0] * raw_bases[0]) + 1e-8
                    initial_scale = numerator / denominator
                    basis_param.data[0] = initial_scale
                    for i in range(1, group["num_bits"]):
                        basis_param.data[i] = initial_scale * 0.001
                    weighted_sum = torch.zeros_like(p.data)
                    for i in range(group["num_bits"]):
                        weighted_sum += basis_param.data[i] * raw_bases[i]
